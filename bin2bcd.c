...
uint16_t bin2bcd(uint16_t binVal){
    union{
        ...
        struct{
            uint8_t     :8;     ///< Valor binário de entrada (bits 7-0)
            uint8_t     :4;     ///< Valor binário de entrada (bits 11-8)
            uint8_t d3  :4;     ///< Dígito 0 (LSB)
            uint8_t d2  :4;     ///< Dígito 1
            uint8_t d1  :4;     ///< Dígito 2
            uint8_t d0  :4;     ///< Dígito 3 (MSB)
            uint8_t     :4;     ///< Bits não usados
        }dat;
    }dataU;
    ...
    for(uint8_t i=0;i<12;i++){  // Quantidades de rodadas igual ao tamanho de dataU.dat.bin
			...
			dataU.shift = dataU.shift<<1; // Desloca dataU.shift para a esquerda uma posição
    }
